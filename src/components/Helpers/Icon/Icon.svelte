<script>
  export let name;
  export let focusable = false;
  export let width = "50";
  export let height = "50";
  let icons = [
    {
      name: "menu",
      box: [35, 24],
      svg: `
          <rect width="35" height="4" rx="2" fill="#2256F2"/>
          <path d="M10 22C10 20.8954 10.8954 20 12 20H33C34.1046 20 35 20.8954 35 22C35 23.1046 34.1046 24 33 24H12C10.8954 24 10 23.1046 10 22Z" fill="#2256F2"/>
          <path d="M0 12C0 10.8954 0.895431 10 2 10H33C34.1046 10 35 10.8954 35 12C35 13.1046 34.1046 14 33 14H2C0.89543 14 0 13.1046 0 12Z" fill="#2256F2"/>
          `,
    },
    {
      name: "close",
      box: [28, 28],
      svg: `
          <rect y="13" x="8" width="3" height="23" rx="2" transform="rotate(-45 0 14)" fill="#2256F2"/>
          <rect y="10" x="27.5" width="3" height="23" rx="2" transform="rotate(45 30 0)" fill="#2256F2"/>
        `,
    },
    {
      name: "down_arrow",
      box: [12, 7],
      svg: `
          <path d="M5.46967 6.53033C5.76256 6.82322 6.23744 6.82322 6.53033 6.53033L11.3033 1.75736C11.5962 1.46447 11.5962 0.989593 11.3033 0.696699C11.0104 0.403806 10.5355 0.403806 10.2426 0.696699L6 4.93934L1.75736 0.696699C1.46447 0.403806 0.989593 0.403806 0.696699 0.696699C0.403806 0.989593 0.403806 1.46447 0.696699 1.75736L5.46967 6.53033ZM5.25 5V6H6.75V5H5.25Z" fill="#2256F2"/>
        `,
    },
    {
      name: "up_arrow",
      box: [16, 9], // maybe 16 9
      svg: `
          <path d="M8.70711 0.292893C8.31658 -0.097631 7.68342 -0.097631 7.29289 0.292893L0.928932 6.65685C0.538408 7.04738 0.538408 7.68054 0.928932 8.07107C1.31946 8.46159 1.95262 8.46159 2.34315 8.07107L8 2.41421L13.6569 8.07107C14.0474 8.46159 14.6805 8.46159 15.0711 8.07107C15.4616 7.68054 15.4616 7.04738 15.0711 6.65685L8.70711 0.292893ZM9 2V1H7V2H9Z" fill="#2256F2"/>
        `,
    },
    {
      name: "linkedin",
      box: [45, 45],
      svg: `
          <path fill="#0288D1" d="M42,37c0,2.762-2.238,5-5,5H11c-2.761,0-5-2.238-5-5V11c0-2.762,2.239-5,5-5h26c2.762,0,5,2.238,5,5V37z"/>
          <path fill="#FFF" d="M12 19H17V36H12zM14.485 17h-.028C12.965 17 12 15.888 12 14.499 12 13.08 12.995 12 14.514 12c1.521 0 2.458 1.08 2.486 2.499C17 15.887 16.035 17 14.485 17zM36 36h-5v-9.099c0-2.198-1.225-3.698-3.192-3.698-1.501 0-2.313 1.012-2.707 1.99C24.957 25.543 25 26.511 25 27v9h-5V19h5v2.616C25.721 20.5 26.85 19 29.738 19c3.578 0 6.261 2.25 6.261 7.274L36 36 36 36z"/>
        `,
    },
    {
      name: "youtube",
      box: [45, 45],
      svg: `
          <rect width="45" height="45" fill="url(#pattern0)"/>
          <defs>
          <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
          <use xlink:href="#image0_221:139" transform="scale(0.00195312)"/>
          </pattern>
          <image id="image0_221:139" width="512" height="512" xlink:href="data:image/png;base64,"/>
          </defs>
        `,
    },
    {
      name: "search",
      box: [20, 21],
      svg: `
          <g clip-path="url(#clip0_134:568)">
          <path d="M7.57615 11.9336L0.505076 19.0047C0.22613 19.2836 0.22613 19.7359 0.505076 20.0148C0.784022 20.2938 1.23628 20.2938 1.51523 20.0148L8.5863 12.9438C8.86524 12.6648 8.86524 12.2126 8.5863 11.9336C8.30735 11.6547 7.85509 11.6547 7.57615 11.9336Z" fill="#8C8C8C"/>
          <path d="M12.8571 13.7143C16.4864 13.7143 19.4286 10.7722 19.4286 7.14287C19.4286 3.51357 16.4864 0.571442 12.8571 0.571442C9.22783 0.571442 6.28571 3.51357 6.28571 7.14287C6.28571 10.7722 9.22783 13.7143 12.8571 13.7143Z" stroke="#8C8C8C" fill="white"/>
          </g>
          <defs>
          <clipPath id="clip0_134:568">
          <rect width="20" height="20.5714" fill="white"/>
          </clipPath>
          </defs>
        `,
    },
  ];
  let displayIcon = icons.find((icon) => icon.name === name);
</script>

<svg
  class={$$props.className}
  {focusable}
  {width}
  {height}
  viewBox="0 0 {displayIcon.box[0]} {displayIcon.box[1]}"
>
  {@html displayIcon.svg}
</svg>

<style>
  /* svg {
      border: 1px solid blue;
    } */
</style>
